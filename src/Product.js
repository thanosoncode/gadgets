   ...
   const checkItem = (id) => {
    ...
    return (
        <div className="product">
                <div className="product-img" onMouseOver={()=>{
                    checkItem(id)
                    setCartPlus(true)
                    ...
                    <button className={cartPlus ? "cart-add-btn show" : "cart-add-btn"} onClick={()=>openModal(id,addedFrom)}>{alreadyInCart ? "already in cart" : <FaCartPlus/>}</button>
                </div>
                <Link to={`/products/${id}`} className="product-link">
                    <h5>{name}</h5>
                </Link>
