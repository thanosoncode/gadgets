                        ...
                        {state.cart.map((item,index)=>{
                            return <tr key={index} className="cart-item-row">
                                ...
                                <td className="cart-item-quantity">
                                    <button onClick={()=>decreaseQuantity(item.id)}><FaMinus/></button><div>{item.quantity}</div><button onClick={()=>increaseQuantity(item.id)}><FaPlus/></button>
                                </td>
                                <td className="cart-item-trash" onClick={()=>removeItem(item.id)}><FaTimes/></td>
                                <td className="cart-item-subtotal">${item.price * item.quantity}.00</td>
                            </tr>
                        })}
                    </tbody>
                </table>
                ...
                <aside>
                    ...
                    <form onSubmit={checkSale}>
                            <div className="promo-div">
                             <input type="text" placeholder="Enter promo code - react" ref={inputEl}/>
